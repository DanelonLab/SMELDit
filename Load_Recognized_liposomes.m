...
%% Parameters to set
experimentDate='2022_06_23'; %Needed to load the right data file
...
%folder that corresponds to the directory that contains the saved cropped liposome images
    folder = ['C:\Users\anasi\OneDrive - Delft University of Technology\Documents\Analysis MATLAB\220623\C1\Recognized Liposomes' sampleName];
    
%% Body of the code 
load(['data_' experimentDate '_' sampleName],'Area','R','dsGreen','dsGreen_var','mCherry','mCherry_var','Cy5','centroids','ImInx','Nimages')
Metrics={'Area','Radius','DsGreen Intensity','DsGreen variance','mCherry intensity','mCherry variance', 'Cy5 intensity'};
varNames={'Area','R','dsGreen','dsGreen_var','mCherry','mCherry_var','Cy5'};
Units={'# pixels', '\mum', 'a.u.', 'a.u.', 'a.u.', 'a.u.', 'a.u.'};
PlotScaleLog=[false false true true true true true];
...
function exportData(src,event)
    ...
    save([datapath datafile],'Area','R','dsGreen','dsGreen_var','mCherry','mCherry_var','Cy5','centroids','ImInx','Nimages','inx','sampleName','folder')
end

function importData(src,event)
    [datafile,datapath] = uigetfile;
    load([datapath datafile],'Area','R','dsGreen','dsGreen_var','mCherry','mCherry_var','Cy5','centroids','ImInx','Nimages','inx','sampleName','folder')
...
function makeMontage(roiMask)
        ...
        for ii=1:Nmontage_used
            jj=ii;
            Ix=inx(jj);
            baseFileName=[sampleName '_00' num2str(ImInx(Ix)) '_' sprintf( '%05d', Ix-im_offset(ImInx(Ix))) '.tif'];
            fullFileName = fullfile(folder, baseFileName);
            filenamesmontage{ii}= fullFileName;
        end
        ...
        figure(2)
        imdisp(filenamesmontage, 'Size', [8 8])%,title(['Showing ' num2str(Nmontage_used) ' example liposomes'])
